#!/bin/bash
# from https://raw.githubusercontent.com/trojan-gfw/trojan-quickstart/master/trojan-quickstart.sh
# license same as https://github.com/trojan-gfw/trojan

# it's funny, i need trojan to cross GFW, but trojan is outside of GFW
...
NAME=trojan
VERSION=$(ls ./trojan*.tar.xz | sed -E 's/.*trojan-(.*)-linux.*/\1/')
TARBALL=$(ls trojan*.tar.xz)
TMPDIR="$(mktemp -d)"
...
echo Copy $NAME $VERSION to temp directory $TMPDIR...
cp $TARBALL $TMPDIR
